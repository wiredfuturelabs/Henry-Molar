...
def rendernotification():
	try:
		if checkversion() != True:
			...
			notifyWindow.config(bg=ttx.rgb(23,27,33))
			notifyWindow.resizable(False,False)
			tk.Label(notifyWindow,text=data[0],bg=ttx.rgb(23,27,33),fg="white",font=("avenir",23)).grid(row=0,column=0,columnspan=2,pady=(20,20),padx=1)

			row = 1
			for messages in data[1:]:
				point = '\u2022'
				tk.Label(notifyWindow,text=('\n%s'+"  "+messages) % point,bg=ttx.rgb(23,27,33),fg="white",font=("avenir",17)).grid(row=row,column=0,columnspan=3,padx=1,sticky=tk.W)
				row += 1

			if checkversion() == False:
				up =tk.Label(notifyWindow,text="Update",bg=ttx.rgb(32,140,255),fg="white",font=("avenir",17))
				up.grid(row=row,column=1,padx=1,pady=30,sticky=tk.E)
